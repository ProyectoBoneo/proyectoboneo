...
def user_is_profesor(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        if request.user.is_profesor or request.user.is_staff:
...
def user_is_alumno(view_func):
    @wraps(view_func, assigned=available_attrs(view_func))
    def _wrapped_view(request, *args, **kwargs):
        if request.user.is_alumno or request.user.is_staff:
            return view_func(request, *args, **kwargs)
        else:
            return HttpResponseForbidden()
...
def user_is_not_alumno(view_func):
    @wraps(view_func, assigned=available_attrs(view_func))
    def _wrapped_view(request, *args, **kwargs):
        if request.user.is_staff or request.user.is_profesor or request.user.is_staff:
            return view_func(request, *args, **kwargs)
        else:
            return HttpResponseForbidden()
